...
const patchUserValidation = [
	body('firstname')
		.optional().notEmpty().withMessage('Firstname is required')
		.custom(value => {
			if(/\s/.test(value)) {
				throw new Error('Firstname must not contain spaces');
			};
	...
	body('lastname')
		.optional().notEmpty().withMessage('Lastname is required')
		.custom(value => {
			if(/\s/.test(value)) {
				throw new Error('Lastname must not contain spaces');
			};
	...
	body('username')
		.optional().notEmpty().withMessage('Username is required')
		.custom(value => {
			if(/\s/.test(value)) {
				throw new Error('Username must not contain spaces');
			};
